...
__global__ void ComputeWavelet(const SampleDesc<T> *sample_data, T begin, T sampling_rate, T b,
  ...
  __global__ void ComputeWavelet(const SampleDesc<T> *sample_data, W<T> wavelet) {
    ...
    for (int i = 0; i < sample.size_b; ++i) {
      const int64_t out_id =
          blockIdx.y * sample.size_b * sample.size_in + i * sample.size_in + t_id;
      auto b = sample.b[i];
  ...
  template <typename T, template <typename> class W>
  DLL_PUBLIC KernelRequirements WaveletGpu<T, W>::Setup(
      KernelContext & context, const InListGPU<T> &a, const InListGPU<T> &b,
      const WaveletSpan<T> &span, const std::vector<T> &args) {
