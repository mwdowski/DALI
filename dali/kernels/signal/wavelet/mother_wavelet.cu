...
__device__ T GaussianWavelet<T>::operator()(const T &t) const {
  ...
  switch (static_cast<int>(n)) {
    ...
    case 4:
      return (-48.0 * std::pow(t, 2.0) + 16.0 * std::pow(t, 4.0) + 12.0) * expTerm /
             std::sqrt(105.0 * sqrtTerm);
    case 5:
      return (-32.0 * std::pow(t, 5.0) + 160.0 * std::pow(t, 3.0) - 120.0 * t) * expTerm /
             std::sqrt(945.0 * sqrtTerm);
    case 6:
      return (-64.0 * std::pow(t, 6.0) + 480.0 * std::pow(t, 4.0) - 720.0 * std::pow(t, 2.0) +
              120.0) *
             expTerm / std::sqrt(10395.0 * sqrtTerm);
    case 7:
      return (128.0 * std::pow(t, 7.0) - 1344.0 * std::pow(t, 5.0) + 3360.0 * std::pow(t, 3.0) -
              1680.0 * t) *
             expTerm / std::sqrt(135135.0 * sqrtTerm);
    case 8:
      return (256.0 * std::pow(t, 8.0) - 3584.0 * std::pow(t, 6.0) + 13440.0 * std::pow(t, 4.0) -
              13440.0 * std::pow(t, 2.0) + 1680.0) *
             expTerm / std::sqrt(2027025.0 * sqrtTerm);
  }
}
