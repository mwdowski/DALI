...
__device__ T MeyerWavelet<T>::operator()(const T &t, const T &a, const T &b) const {
  ...
  T psi1 = (4 / (3 * M_PI) * x * std::cos((2 * M_PI) / 3 * x) -
            1 / M_PI * std::sin((4 * M_PI) / 3 * x)) /
           (x - 16 / 9 * std::pow(x, 3.0));
  T psi2 =
      (8 / (3 * M_PI) * x * std::cos(8 * M_PI / 3 * x) + 1 / M_PI * std::sin((4 * M_PI) / 3) * x) /
      (x - 64 / 9 * std::pow(x, 3.0));
...
__device__ T MexicanHatWavelet<T>::operator()(const T &t, const T &a, const T &b) const {
  T x = std::pow(2.0, a) * t - b;
  return std::pow(2.0, a / 2.0) *
         (2 / (std::sqrt(3 * sigma) * std::pow(M_PI, 0.25)) * (1 - std::pow(x / sigma, 2.0)) *
...
__device__ T FbspWavelet<T>::operator()(const T &t, const T &a, const T &b) const {
  T x = std::pow(2.0, a) * t - b;
  return std::pow(2.0, a / 2.0) *
         (std::sqrt(fb) * std::pow(sinc(x / std::pow(fb, m)), m) * std::exp(2 * M_PI * fc * x));
}
